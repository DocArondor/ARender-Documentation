...
Cette configuration a lieu dans le fichier `arender.xml` dans le bean *xfdfAnnotationAccessor* et la propriété *annotationCreationPolicy*.

    =========================================   =============================   =========================================
    Description                                 Clé du paramètre                Type
    =========================================   =============================   =========================================
    Autoriser à créer des annotations           canCreateAnnotations            Booléen
    Notes textuelles supportent le HTML         textAnnotationsSupportHtml      Booléen
    Notes textuelles supportent les réponses    textAnnotationsSupportReply     Booléen
    Notes textuelles supportent les statuts     textAnnotationsSupportStatus    Booléen
    Annotations supportent la sécurité          annotationsSupportSecurity      Booléen
    Liste des sécurités                         availableSecurityLevels         Liste de SecurityLevel
    Liste des modèles de tampon                 annotationTemplateCatalog       AnnotationTemplateCatalog
    =========================================   =============================   =========================================

Voir les parties correspondantes pour la configuration de :ref:`la sécurité des annotations <AnnotationSecurity>` et :ref:`les modèles de tampon <Stamp>`.
...
Afin de modifier les tampons existants, ou bien en rajouter de nouveaux, modifier la configuration existante dans `annotation-template-catalog.xml`.
Deux types de tampons sont disponibles, les tampons texte et image. 
...
Il est possible de surcharger des propriétés d'annotation définies par défaut dans `arender-default.properties`, comme par exemple l'opacité ou bien la couleur.
La configuration a lieu dans `events-configuration.xml` dans un bean d'action de création (class = `com.arondor.viewer.client.toppanel.behavior.annotation.CreateAnnotationButtonHandler`)
Pour cela il faut définir en plus du type d'annotation à créer, un modèle d'annotation.
...
La liste des sécurités permet de définir les sécurités que l'on souhaite pour toutes les annotations.
Cette liste sera visible en tant que liste déroulante dans la barre d'outils lors de l'édition d'une annotation.
